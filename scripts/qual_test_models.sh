...
python -m scripts.test_and_fuse --prob_threshold 0.1 --disp_threshold 0.2 --num_consistent 3 \
--test_folder_root $TEST_DIR --results_path $RESULTS_PATH \
--model_dir gs://mvs-training-mlengine/f_alpha_025_beta_0_continuation/models/ --ckpt_step 340000


python -m scripts.test_and_fuse --prob_threshold 0.1 --disp_threshold 0.2 --num_consistent 3 \
--test_folder_root $TEST_DIR --results_path $RESULTS_PATH \
--model_dir gs://mvs-training-mlengine/f_gaussian_eta_0_02/models/ --ckpt_step 140000
