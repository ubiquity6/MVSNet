...
python -m mvsnet.test --input_dir=$TEST_DIR --results_path=$RESULTS_PATH --wandb \
--model_dir=gs://mvs-training-mlengine/f_grad_loss_alpha_0_25_beta_0/models/ \
--ckpt_step=135000


python -m mvsnet.test --input_dir=$TEST_DIR --results_path=$RESULTS_PATH --wandb \
--model_dir=gs://mvs-training-mlengine/f_gaussian_eta_0_02/models/ \
--ckpt_step=140000

python -m mvsnet.test --input_dir=$TEST_DIR --results_path=$RESULTS_PATH --wandb \
--model_dir=gs://mvs-training-mlengine/f_gaussian_eta_0_04/models/ \
--ckpt_step=140000
